// gradient.js — multi-stop hex color gradient sampler.
// Based on javascript-color-gradient by Lasha Kakabadze (@Adrinlol),
  ...
  constructor(startColor = '', endColor = '', minNum = 0, maxNum = 10) {
    ...
    this.getColor = (numberValue) => {
      if (numberValue) {
        return (
          '#' +
          generateHex(numberValue, startColor.substring(0, 2), endColor.substring(0, 2)) +
          generateHex(numberValue, startColor.substring(2, 4), endColor.substring(2, 4)) +
          generateHex(numberValue, startColor.substring(4, 6), endColor.substring(4, 6))
        );
      }
    };
